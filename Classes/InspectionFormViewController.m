...
- (void)viewDidLoad {
    ...
    self.originalFeatureAttributes = [self.inspectionEditingPopup.graphic.attributes mutableCopy];
    ...
    NSArray *inspectionFieldNames = [self.inspectionEditingPopup.graphic.attributes allKeys];
    NSArray *featureFieldNames = [self.featureToInspectPopup.graphic.attributes allKeys];
    ...
    for (NSString *fieldName in featureFieldNames) {
        if ([inspectionFieldNames containsObject:fieldName] && ![offLimitsFieldNames containsObject:fieldName]) {
            id featureValue = [self.featureToInspectPopup.graphic.attributes objectForKey:fieldName];
            [self.inspectionEditingPopup.graphic.attributes setObject:featureValue forKey:fieldName];
        }
    }
    ...
    for (AGSPopupFieldInfo *fi in self.inspectionEditingPopup.popupInfo.fieldInfos)
    {
        ...
        if ([self.attributeUtility isADateField:field]) {
            ...
            [self.inspectionEditingPopup.graphic.attributes setObject:dateNum forKey:fi.fieldName];
        }
    }
    ...
    for (AGSPopupFieldInfo *fi in self.featureToInspectPopup.popupInfo.fieldInfos){
        ...
        if ([self.attributeUtility isAStringField:field]) {
            ...
            NSString *firstString = [self.featureToInspectPopup.graphic.attributes objectForKey:fi.fieldName];
...
-(void)domainPickerDidFinish:(DomainPickerViewController *)dpvc
{
    if (self.attributeUtility.featureLayer){
		if ([dpvc.fieldInfo.fieldName isEqualToString:self.attributeUtility.featureLayer.typeIdField]){
			if (![dpvc.value isEqual:[self.inspectionEditingPopup.graphic.attributes objectForKey:dpvc.fieldInfo.fieldName]]){
    ...
    [self.inspectionEditingPopup.graphic.attributes setValue:dpvc.value forKey:dpvc.fieldInfo.fieldName];
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    self.domainPickerVC.value = [self.inspectionEditingPopup.graphic.attributes objectForKey:fi.fieldName];
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if ([self.attributeUtility isAStringField:field]) {
        ...
        [self.inspectionEditingPopup.graphic.attributes setObject:text forKey:fi.fieldName];
    }
    else if([self.attributeUtility isANumberField:field])
    {
        id numValue = [self getValueFromTextField:textField forFeatureType:field.type];
        
        [self.inspectionEditingPopup.graphic.attributes setObject:numValue forKey:fi.fieldName];
    }
